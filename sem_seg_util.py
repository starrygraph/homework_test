...
def parse_args():
    ...
    parser.add_argument('--batch_size', type=int, default=8,
                        help='Batch Size during training for each GPU [default: 8]')
    ...
    parser.add_argument('--bn_init_decay', type=float, default=0.5,
                        help='Initial decay rate for bn decay [default: 0.5]')
    parser.add_argument('--bn_decay_decay_rate', type=float, default=0.5,
                        help='BN decay rate for bn decay [default: 0.5]')
    parser.add_argument('--bn_decay_decay_step', type=int, default=300000,
                        help='BN decay rate decay step [default: 300000]')
    parser.add_argument('--bn_decay_clip', type=float, default=0.99, help='BN decay clip [default: 0.99]')

    parser.add_argument('--num_neighbors', type=int, nargs='+', default=[16],
                        help='The number of k nearest neighbors [Default: 16]. You can either pass a single value for all layers or one value per layer.')
    parser.add_argument('--num_filters', type=int, nargs='+', default=[64],
                        help='The number of filers in gcn layers [default: 64]')
    parser.add_argument('--dilations', type=int, nargs='+', default=[-1],
                        help='The dilation rate for each layer [default: -1 => dilation rate = layer number]')
    add_bool_arg(parser, 'stochastic_dilation', default=True)
    parser.add_argument('--sto_dilated_epsilon', type=float, default=0.2,
                        help='Stochastic probability of dilatioin [Default: 0.2]')
    parser.add_argument('--skip_connect', type=str, default='residual',
                        help='Skip Connections (residual, dense, none) [default: residual]')
    parser.add_argument('--edge_lay', type=str, default='dilated',
                        help='The type of edge layers (dilated, knn) [default: dilated]')
    parser.add_argument('--gcn', type=str, default='edgeconv',
                        help='The type of GCN layers (mrgcn, edgeconv, graphsage, gin) [default: edgeconv]')
...
def load_data(all_files, room_filelist, test_area_idx):
    ...
    return data_batches[train_idxs, ...], label_batches[train_idxs], data_batches[test_idxs, ...], label_batches[
        test_idxs]
